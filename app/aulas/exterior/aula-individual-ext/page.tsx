...
import { CheckCircle2, MessageCircle, Zap, Globe2, Sparkles, Globe, Info, Users2 } from 'lucide-react';
import Link from 'next/link';

const AulaIndividualExtContent = () => {
    const comparisonData = [
        ...
        },
        {
            ...
            time: "MAIOR DISPONIBILIDADE DE HORÁRIOS"
        },
        {
            label: "PLANO DE AULA",
        ...
        {
            label: "PAGAMENTO",
            lila: "INTEGRAL E ADIANTADO",
            time: "INTEGRAL E ADIANTADO"
        }
    ];

    const lilaPlans = [
        { qty: "20 horas", price: "560€" },
        { qty: "30 horas", price: "750€", popular: true },
        { qty: "40 horas", price: "840€" },
    ];

    const timePlans = [
        { qty: "21 horas", price: "420€", bonus: "Leve 1 aula GRÁTIS" },
        { qty: "32 horas", price: "570€", bonus: "Leve 2 aulas GRÁTIS", popular: true },
        { qty: "43 horas", price: "720€", bonus: "Leve 3 aulas GRÁTIS" },
    ];
            ...
            <section className="py-12 md:py-20 bg-white overflow-hidden">
                <div className="container mx-auto px-4">
                    ...
                    <div className="flex flex-col lg:flex-row items-center gap-16 mb-32">
                        ...
                        >
                            <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-gold/10 text-gold rounded-full text-xs font-black uppercase tracking-widest mb-6">
                                <Globe2 size={14} /> Foco Individual
                            </div>
                            ...
                            <p className="text-xl text-slate-600 mb-10 leading-relaxed max-w-lg">
                                Currículo moldado 100% nos seus objetivos, no seu ritmo e adaptado ao seu fuso horário. Ideal para quem mora no exterior.
                            </p>
                            <div className="space-y-5 text-slate-700">
                                {["Avança no seu tempo, sem depender de ninguém", "Encontros de 50-60min via Meet ou Zoom", "Suporte direto no meu WhatsApp para dúvidas", "Pagamento internacional facilitado", "Material e atividades pensados só para você"].map((item, i) => (
                        ...
                        <div className="mb-16 relative">
                            ...
                            <div className="mt-6 flex flex-col items-center justify-center gap-5">
                                <p className="text-[11px] text-slate-500 font-bold uppercase tracking-widest flex items-center gap-3">
                                    <span className='flex gap-1'><Globe size={15} /> Alunos no Exterior</span>
                                </p>

                                {/* BADGES DE DESCONTO - ESTILO PM (ALTO CONTRASTE / CLEAN) */}
                                <div className="flex flex-wrap justify-center gap-3">
                                    <span className="inline-flex items-center gap-2 px-4 py-2 bg-gold/5 text-gold border border-gold/20 rounded-full text-[10px] font-black uppercase tracking-tight shadow-sm transition-all hover:bg-gold/10">
                        ...
                        <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
                            {lilaPlans.map((plan, i) => (
                                ...
                                >
                                    ...
                                    <h3 className="text-3xl font-bold text-navy mb-4 uppercase tracking-tight">{plan.qty}</h3>
                                    <div className="text-5xl font-black text-navy mb-8 tracking-tighter">{plan.price}</div>
                                    <div className="mt-auto pt-8 border-t border-slate-50">
                                        <Link href={`/contrato?plano=${encodeURIComponent(plan.qty + " com Lila - Exterior")}`}>
                                            <button className="w-full py-5 bg-navy text-white rounded-2xl font-bold hover:bg-gold hover:text-navy transition-all uppercase text-xs tracking-widest cursor-pointer">Garantir vaga</button>
                                        </Link>
                                    </div>
                                </motion.div>
                            ))}
                        </div>
                        <p className="mt-12 text-slate-400 text-[10px] font-bold uppercase tracking-[0.2em] flex items-center justify-center gap-2">
                            <Info size={14} className="text-gold" />
                            Pagamento integral e adiantado via PayPal, Wise ou Transferência Internacional
                        </p>
                    </div>
                        ...
                        <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
                            {timePlans.map((plan, i) => (
                                <motion.div
                                    key={i}
                                    whileHover={{ y: -10 }}
                                    className={`p-10 rounded-[2.5rem] border ${plan.popular ? 'border-navy bg-slate-50 shadow-lg scale-105 relative z-10' : 'border-slate-200 bg-white'} text-center transition-all flex flex-col`}
                                >
                                    {plan.popular && <span className="absolute -top-4 left-1/2 -translate-x-1/2 bg-navy text-white text-[10px] font-black px-4 py-1 rounded-full uppercase">Melhor Escolha</span>}
                                    ...
                                    <div className="mt-auto pt-8 border-t border-slate-100">
                                        <Link href={`/teachers?plano=${encodeURIComponent(plan.qty + " com Time - Exterior")}`}>
                    ...
                    <div className="w-full max-w-6xl mx-auto mb-32">
                        <div className="text-center mb-16">
                            <h2 className="text-4xl md:text-6xl font-black text-navy tracking-tighter mb-4">
                                Lila ou <span className="text-gold italic font-serif font-normal">Time?</span>
                            </h2>
                        </div>
                            ...
                            <div className="flex flex-col lg:flex-row gap-16 items-center relative z-10">
                                <div className="lg:w-2/3 text-left">
                                    <div className="flex flex-wrap items-center gap-4 mb-8">
                                        <span className="px-4 py-1.5 bg-gold text-navy font-black rounded-lg text-[10px] uppercase tracking-widest shadow-sm">Foco & Agilidade</span>
                                    ...
                                    <p className="text-slate-600 text-xl mb-10 leading-relaxed italic">Sessões de 30 minutos para quem já fala (B1+) e precisa manter o inglês afiado.</p>
                                    <div className="flex flex-wrap gap-12">
                                        <div><p className="text-[10px] font-black text-slate-300 uppercase tracking-widest mb-2">Mensal / 4x</p><p className="text-4xl font-bold text-navy tracking-tighter">79,99€</p></div>
                                        <div className="h-12 w-px bg-slate-100 hidden sm:block" />
                                        <div><p className="text-[10px] font-black text-slate-300 uppercase tracking-widest mb-2">Plano 20</p><p className="text-4xl font-bold text-navy tracking-tighter">359,99€</p></div>
                                    </div>
                                    <p className="mt-8 text-slate-400 text-[9px] font-bold uppercase tracking-widest italic">
                                        *Pagamento integral via Wise/PayPal
                                    </p>
                                </div>
                                <div className="lg:w-1/3 w-full">
                                    <Link href={`/contrato?plano=${encodeURIComponent("Inglês On The Go (OTG) - Exterior")}`}>
                    ...
                    <div className="text-center pt-10">
                        ...
                        <a
                            href="https://wa.me/19787939832?text=Olá Lila! Tenho dúvidas sobre os planos individuais para quem mora no exterior."
...
export default AulaIndividualExtContent;