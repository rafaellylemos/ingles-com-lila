...
export async function POST(req: Request) {
  try {
    ...
    console.log(`[LOG] Processando Lista de Espera: PREZÃO (A2) | Aluno: ${nome}`);

    const { data, error } = await resend.emails.send({
      from: 'Lista de Espera Lila <onboarding@resend.dev>',
      to: ['rafaellyltbmarques@gmail.com'],
      subject: `⚡ [LISTA ESPERA] PREZÃO (A2) - ${nome}`,
      replyTo: email,
      html: `
        <div style="font-family: sans-serif; line-height: 1.6; color: #1a1a1a; max-width: 600px; margin: 0 auto; border: 1px solid #eee; padding: 30px; border-radius: 20px; border-top: 5px solid #D4AF37;">
          <h2 style="color: #002147; padding-bottom: 10px; margin-bottom: 20px;">
            Novo Cadastro: Nível Pré-Intermediário (A2)
          </h2>
          ...
          <div style="background-color: #f9f9f9; padding: 20px; border-radius: 15px; border: 1px solid #f0f0f0;">
            ...
            <p style="margin: 8px 0;"><strong>📚 Grupo de Interesse:</strong> <span style="color: #D4AF37; font-weight: bold;">PREZÃO (A2)</span></p>
          </div>
          
          <div style="margin-top: 25px; padding: 15px; background-color: #fff9e6; border-radius: 10px; border-left: 4px solid #D4AF37;">
             <p style="margin: 0; font-size: 14px; color: #856404;">
               <strong>Dica do Sistema:</strong> Este aluno quer sair do "inglês engessado". Responda diretamente este e-mail para iniciar o contato comercial.
             </p>
          </div>
    ...
    if (error) {
      console.error("Erro técnico do Resend no Prezão:", error);
      return NextResponse.json({ error }, { status: 403 });
    }

    return NextResponse.json({
      success: true,
      message: "Inscrição para o Prezão realizada com sucesso!",
      id: data?.id
    });

  } catch (error) {
    console.error("Erro interno no servidor (Prezão):", error);
    return NextResponse.json(
